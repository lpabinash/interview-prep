    ...
    <header className="sticky top-0 z-40 bg-background dark:bg-[#0A0A0A] border-b border-border-light dark:border-[#2A2A2A]">
      <div className="flex items-center justify-between pl-14 pr-6 md:px-12 py-3">
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          {/* Course CTA links — hidden on mobile, shown on lg+ */}
          {[
            { href: '/react', label: 'React' },
            { href: '/nodejs', label: 'Node' },
            { href: '/dsa', label: 'DSA' },
            { href: '/fsd', label: 'FSD' },
            { href: '/typescript', label: 'TS' },
            { href: '/nextjs', label: 'Next' },
          ].map(({ href, label }) => (
            <a
              key={href}
              href={href}
              className="hidden lg:inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono tracking-wide border border-foreground dark:border-[#FAFAFA] hover:bg-foreground hover:text-background dark:hover:bg-[#FAFAFA] dark:hover:text-[#0A0A0A] transition-colors duration-100"
            >
              {label} →
            </a>
          ))}
