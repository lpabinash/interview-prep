...
if (fs.existsSync(JS_CONTENT_ROOT)) {
  try {
    const entries = fs.readdirSync(JS_CONTENT_ROOT, { withFileTypes: true })
    ...
    for (const dirName of chapterDirs) {
      ...
      let content = ''
      try {
        const raw = fs.readFileSync(path.join(JS_CONTENT_ROOT, dirName, 'README.md'), 'utf-8')
    ...
    // Concepts
    const conceptsDir = path.join(JS_CONTENT_ROOT, 'Concepts')
    ...
    console.warn(`⚠ Error processing javascript-chapters:`, err.message)
  }
}
...
if (fs.existsSync(REACT_CONTENT_ROOT)) {
  try {
    ...
    console.warn(`⚠ Error processing react-chapters:`, err.message)
  }
}
...
if (fs.existsSync(NODE_CONTENT_ROOT)) {
  try {
    ...
    console.warn(`⚠ Error processing nodejs-chapters:`, err.message)
  }
}
...
if (fs.existsSync(DSA_CONTENT_ROOT)) {
  try {
    ...
    console.warn(`⚠ Error processing dsa-chapters:`, err.message)
  }
}
...
if (fs.existsSync(FSD_CONTENT_ROOT)) {
  try {
    ...
    console.warn(`⚠ Error processing fsd-chapters:`, err.message)
  }
}
...
if (fs.existsSync(TS_CONTENT_ROOT)) {
  try {
    ...
    console.warn(`⚠ Error processing typescript-chapters:`, err.message)
  }
}
...
if (fs.existsSync(NEXTJS_CONTENT_ROOT)) {
  try {
    ...
    console.warn(`⚠ Error processing nextjs-chapters:`, err.message)
  }
}
