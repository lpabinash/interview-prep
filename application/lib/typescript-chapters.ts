...
function parseDirName(dirName: string): { number: string; title: string } | null {
  const match = dirName.match(TS_CHAPTER_DIR_RE)
...
function getAllTsChapterDirs(): string[] {
  if (!fs.existsSync(TS_CONTENT_ROOT)) return []
  const entries = fs.readdirSync(TS_CONTENT_ROOT, { withFileTypes: true })
  return entries
    .filter((e) => e.isDirectory() && TS_CHAPTER_DIR_RE.test(e.name))
    .map((e) => e.name)
    .sort((a, b) => {
...
function getReadmeContent(dirName: string): string | null {
  const filePath = path.join(TS_CONTENT_ROOT, dirName, 'README.md')
...
export function getAllTsChapters(): ChapterMeta[] {
  const chapterDirs = getAllTsChapterDirs()
  ...
  return chapterDirs.map((dirName) => {
    ...
    return {
      ...
      season: 1 as const,
      seasonLabel: 'TypeScript',
      readTime,
    }
  })
}

export function getTsChapterBySlug(slug: string): Chapter | null {
  const chapterDirs = getAllTsChapterDirs()
...
export function getTsSeasons(): Season[] {
  const allChapters = getAllTsChapters()
  ...
  if (basics.length > 0) {
    seasons.push({
      number: 1,
      label: 'Fundamentals',
      description: 'TypeScript basics — type system, functions, interfaces, unions, and intersections',
      chapters: basics,
    })
  }
  ...
  if (intermediate.length > 0) {
    seasons.push({
      number: 2,
      label: 'Intermediate',
      description: 'Generics, type narrowing, utility types, and advanced type system features',
  ...
  if (advanced.length > 0) {
    seasons.push({
      number: 3,
      label: 'Advanced',
      description: 'Enums, modules, DOM integration, configuration, and best practices',
      chapters: advanced,
    })
  }
