...
import { getAllTsChapters, getTsChapterBySlug } from '@/lib/typescript-chapters'
...
export function generateStaticParams() {
  const chapters = getAllTsChapters()
...
export async function generateMetadata({
  ...
  const chapter = getTsChapterBySlug(slug)
  if (!chapter) return { title: 'Chapter Not Found' }
  return {
    title: `${chapter.title} — Namaste TypeScript`,
    description: `Namaste TypeScript — ${chapter.number}: ${chapter.title}`,
  }
}

export default async function TsChapterPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  const allSlugsInSeason = allChapters.map((c) => c.slug)
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-16 md:py-24">
        <header className="mb-12">
          <div className="flex flex-col gap-0.5 mb-3">
            <span className="font-mono text-xs tracking-widest uppercase text-accent">
              Namaste TypeScript · {chapter.number}
            </span>
          ...
          <div className="mt-4 flex items-center gap-3 flex-wrap">
            <BookmarkButton slug={chapter.slug} course="typescript" />
            <CompleteButton slug={chapter.slug} seasonLabel="TypeScript" allSlugsInSeason={allSlugsInSeason} />
            <PrintButton />
          </div>
        </header>
        ...
        <ChapterNav prev={prev} next={next} chapterBasePath="/typescript/chapters" />
      </div>
    </>
  )
}
