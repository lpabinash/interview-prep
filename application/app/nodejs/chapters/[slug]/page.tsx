...
import { getAllNodeChapters, getNodeChapterBySlug } from '@/lib/node-chapters'
...
export function generateStaticParams() {
  const chapters = getAllNodeChapters()
...
export async function generateMetadata({
  ...
  const chapter = getNodeChapterBySlug(slug)
  if (!chapter) return { title: 'Episode Not Found' }
  return {
    title: `${chapter.title} — Namaste Node.js`,
    description: `Namaste Node.js — ${chapter.seasonLabel} ${chapter.number}: ${chapter.title}`,
  }
}

export default async function NodeChapterPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-16 md:py-24">
        <header className="mb-12">
          ...
          <div className="mt-4 flex items-center gap-3 flex-wrap">
            <BookmarkButton slug={chapter.slug} course="nodejs" />
        ...
        <ChapterNav prev={prev} next={next} chapterBasePath="/nodejs/chapters" />
      </div>
    </>
  )
}
