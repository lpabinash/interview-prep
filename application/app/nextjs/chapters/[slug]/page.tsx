...
import { getAllNextjsChapters, getNextjsChapterBySlug } from '@/lib/nextjs-chapters'
...
export function generateStaticParams() {
  const chapters = getAllNextjsChapters()
...
export async function generateMetadata({
  ...
  const chapter = getNextjsChapterBySlug(slug)
  if (!chapter) return { title: 'Chapter Not Found' }
  return {
    title: `${chapter.title} — Namaste Next.js`,
    description: `Namaste Next.js — ${chapter.number}: ${chapter.title}`,
  }
}

export default async function NextjsChapterPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-16 md:py-24">
        <header className="mb-12">
          <div className="flex flex-col gap-0.5 mb-3">
            <span className="font-mono text-xs tracking-widest uppercase text-accent">
              Namaste Next.js · {chapter.number}
            </span>
          ...
          <div className="mt-4 flex items-center gap-3 flex-wrap">
            <BookmarkButton slug={chapter.slug} course="nextjs" />
            <CompleteButton slug={chapter.slug} seasonLabel="Next.js" allSlugsInSeason={allSlugsInSeason} />
            <PrintButton />
          </div>
        </header>
        ...
        <ChapterNav prev={prev} next={next} chapterBasePath="/nextjs/chapters" />
      </div>
    </>
  )
}
