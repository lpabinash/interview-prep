...
import { getAllFsdChapters, getFsdChapterBySlug } from '@/lib/fsd-chapters'
...
export function generateStaticParams() {
  const chapters = getAllFsdChapters()
...
export async function generateMetadata({
  ...
  const chapter = getFsdChapterBySlug(slug)
  if (!chapter) return { title: 'Topic Not Found' }
  return {
    title: `${chapter.title} — Namaste Frontend System Design`,
    description: `Namaste FSD — ${chapter.seasonLabel} · ${chapter.number}: ${chapter.title}`,
  }
}

export default async function FsdChapterPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-16 md:py-24">
        <header className="mb-12">
          ...
          <div className="mt-4 flex items-center gap-3 flex-wrap">
            <BookmarkButton slug={chapter.slug} course="fsd" />
        ...
        <ChapterNav prev={prev} next={next} chapterBasePath="/fsd/chapters" />
      </div>
    </>
  )
}
