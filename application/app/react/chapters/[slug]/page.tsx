...
import { getAllReactChapters, getReactChapterBySlug } from '@/lib/react-chapters'
...
export function generateStaticParams() {
  const chapters = getAllReactChapters()
...
export async function generateMetadata({
  ...
  const chapter = getReactChapterBySlug(slug)
  if (!chapter) return { title: 'Chapter Not Found' }
  return {
    title: `${chapter.title} — Namaste React`,
    description: `Namaste React — ${chapter.number}: ${chapter.title}`,
  }
}

export default async function ReactChapterPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-16 md:py-24">
        <header className="mb-12">
          ...
          <div className="mt-4 flex items-center gap-3 flex-wrap">
            <BookmarkButton slug={chapter.slug} course="react" />
            <CompleteButton slug={chapter.slug} seasonLabel="React" allSlugsInSeason={allSlugsInSeason} />
            <PrintButton />
          </div>
        </header>
        ...
        <ChapterNav prev={prev} next={next} chapterBasePath="/react/chapters" />
      </div>
    </>
  )
}
